...
RUN mkdir /tmp/weewx-dwd/ &&\
    ...
    sed -i -z -e "s|#/usr/local/bin/dwd-cap-warnings --weewx --resolution=city 2>/dev/null >/dev/null|/usr/local/bin/dwd-cap-warnings --config=/data/weewx.conf --weewx --resolution=city 2>/dev/null >/dev/null|g" /etc/cron.hourly/dwd &&\
    sed -i -z -e "s|/usr/local/bin/dwd-mosmix --weewx --daily --hourly XXXXX 2>/dev/null >/dev/null|/usr/local/bin/dwd-mosmix --config=/data/weewx.conf --weewx --daily --hourly P444 2>/dev/null >/dev/null|g" /etc/cron.hourly/dwd
